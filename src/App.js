...
class App extends Component {
  ...
  callApi = async params => {
    let response = await params.map(async el => {
      //const response = await fetch(window.encodeURI(`http://localhost:5000/api/twitterSearch/`+JSON.stringify(el)));
      const response = await fetch(
        window.encodeURI(`https://us-central1-bitcoin-weekly-sentiment.cloudfunctions.net/api/twitterSearch?content=${JSON.stringify(el)}`)
      );
